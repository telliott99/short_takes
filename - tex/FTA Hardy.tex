...
The right hand side is $n$ so $p_1 \cdot p_1 < n$.  A similar result holds for $q_1$.
...
We also know that neither $p_1$ nor $q_1$ is equal to zero, $0 < p_1 \cdot q_1$.  So we can write:
\[ 0 < p_1 \cdot q_1 < n \]
...
Both $p_1$ and $q_1$ appear in the factorization of $N$, and that factorization is unique, since $N$ is not abnormal.  Therefore, $p_1$ times $q_1$ \emph{also} divides $N$.  
