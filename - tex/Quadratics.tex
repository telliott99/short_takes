...
Call the new constants $b'$ and $c'$
\[ x^2 + b'x + c' = 0 \]
This new equation has a different shape ($a$ is called the shape factor).  We have effectively re-scaled $y$ by the factor $1/a$.

For the new curve (colored blue) in the figure below, if you go out $1$ unit from the vertex, change $x$ by $\pm 1$, the curve crosses through a point $1$ unit above that.  If $\Delta x$ is $\pm 2$, $\Delta y$ will be $4$.

Nevertheless, the roots of the equation, the values of $x$ that make this equation true, are unchanged.  We can see this in an example:
...
We can see that this equality holds precisely when $x = s$ or $x = t$.  The connection with the previous equation can be discovered by multiplying out
\[ x^2 - sx -tx + st = 0 \]
\[ x^2 - (s + t)x + st = 0 \]
...
The mean, or average, of $s$ and $t$ is equal to $-b'/2$, which is equal to $-b/2a$.  
...
$\circ$ \ divide through by $a$ to get $x^2 + b'x + c' = 0$.
