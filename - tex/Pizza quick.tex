...
Next, consider ``horizontal" movement along a chord, but not on a diagonal of the circle.  We focus on a particular chord with arms $t$ and $s$ (one chord is not shown).  Movement of the grid center is to the right, magenta arrow.  

The circle center is in red, and the second black dot marks the center of the chord.  We make the following observations:

$\circ$ \ The chord center is constrained to fall on the red line, the perpendicular bisector.  The bisector never changes, only the position of the chord center on it.  This follows from the fact that in all movements the chord doesn't change angle, the final is parallel to the beginning position.

$\circ$ \ Distance from chord center to circle center changes by the amount of the movement, with an extra factor of $\sqrt{2}$ imposed by the geometry.

$\circ$ \ If the region above arm $t$ is ``shaded", then that below arm $s$ is unshaded.  So the net change in shaded area is $t$ \emph{minus} $s$.

$\circ$ \ The curved shapes formed by the movement of a chord, at the ends of any individual rectangle, are mirror images.  Since one changes the shaded and the other the unshaded area, they cancel.
\begin{center} \includegraphics [scale=0.35] {pizza13.png} \end{center}
...
We can show that the net change in those latter distances for the two chords considered together, is zero.  Therefore the change in shaded area is also zero.

Start with the difference between the long and the short arm of any chord.  We can find the distance from the center of that chord to the grid center as one-half of that distance.  

Call that distance $\delta$.  Take what was above $\Delta A = 0$, and divide both sides by $2$.  This gives an \emph{invariant} which obviously holds when the grid center remains on a diagonal of the circle (vertical movement).  In fact, by symmetry $\delta_{cd} = \delta_{gh}$ so 
...
Horizontal movement \emph{does} change $\delta_{cd}$ and $\delta_{gh}$, but they are adjacent sides in a rectangle whose four vertices are the chord centers, the circle center and the grid center.  It is a rectangle because the two chords are perpendicular and the red lines are perpendicular bisectors.

Both chords move along fixed lines connected to the circle center (red lines).  The distance each moves for any given horizontal translation is the same (with opposite sign), because each center moves along the hypotenuse of an isosceles right triangle, whose base has the magnitude of the movement.
\begin{center} \includegraphics [scale=0.4] {pizza12.png} \end{center}
...
It is possible to reach any position and orientation by vertical plus horizontal movement.  (\emph{Proof}:  start with any position and perform a horizontal translation followed by vertical movement along a diagonal of the circle.  Now reverse the motion.)
