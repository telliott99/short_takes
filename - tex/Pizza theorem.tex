...
Consider a circular pizza pie.  Choose a point \emph{anywhere} in the pie.  Draw two chords crossing at the point, at right angles with any orientation, and then fill in with two more chords that bisect the angles ($45^{\circ}$).
...
Now, slide the grid center away from the circle center along a diagonal.  Because the circle is featureless, we can pick the vertical diagonal without loss of generality, as they say.

In the right panel, the grid center has moved vertically down from the circle center.  The circle center is marked with a magenta dot.
...
Having reached a specified distance from the circle center, then spin radially about the grid center to achieve the desired final orientation.  Note in passing that performing the moves in the opposite sequence cannot reach every position, because the grid center must remain on a diagonal of the circle.
...
So the crux of the analysis is horizontal motion or radial motion.  We will take a first look at vertical motion along a diagonal, and then proceed to others in turn.  But even before that, we need to do some preparatory work.

\subsection*{extraordinary property}

Recall that if we have a chord $PR$ in a circle on center $O$, and $PR$ is parallel to the tangent line drawn through $T$, then the angles at $S$ are right angles.
\begin{center}
...
So then, let's say we have a rectangle in a circle, but only one side is a chord of the circle.  Draw the vertical bisector of side $AC$.
\begin{center} \includegraphics [scale=0.4] {perp_chords9.png} \end{center}
Chord $DH$ contains the opposite side, $EG$, plus two extensions.  $EG$ since it is equal and parallel to $DH$, is also bisected by $OB$.  $OB$ bisects $DH$.  By subtracting equals from equals, we have that $DE = GH$:  the extensions are equal.

Now consider two chords at right angles in a circle.  Form the rectangle using a side parallel to chord $a + b$.  (Only two sides are shown).  An easy way to do this is to draw the two diagonals (dotted lines).
...
There is also a preliminary result which will be helpful in solving the problem, specifically the invariance of areas under translation parallel to one of the chords.
...
In the diagram, the red dot is the center of the circle.  Because of the right angles, the distance from (the center of) chord $c+d$ to the center of the circle is the same as the distance from the grid center to the center of perpendicular chord $g + h$, and vice versa.  Read that again if you need to.
...
\[ 2ab = 2cd \]
\
But of course this is obvious follows from similar triangles so
\[ \frac{a}{c} = \frac{d}{b} \]
...
Changes to the wingtips of a chord always cancel.  For example, in horizontal movement, region I loses the wingtip from $d$, while region III gains the one from $c$.  One is added and the other subtracted from a region of the same shading, so these changes to area cancel.  

Referring to just the rectangles by their lengths (i.e. minus the wings):
...
What remains to be considered is simply areas of rectangles.  The vertical rectangle is wider than the others. Because the movement is parallel to its width, it captures more area.

The ratio of areas is $\sqrt{2}$.  That is, the width of the rectangle formed from the chord $a + b$ is larger than the width of the rectangle formed from chord $c+d$ by $\sqrt{2}$, because the latter is angled at $45^{\circ}$ to the direction of travel.
...
First, this invariant holds when the grid center remains on a diagonal of the circle (vertical movement).
...
I am not entirely happy with this argument either.  The reason is that the oddly shaped regions at the ends of the arcs are different for the different chords.  My intuition is that this means the functions $r_i (\phi_i)$ in the integral are \emph{different}.  This is problematic.
