...
The exponent is a new notation we introduce where rather than write $2 \cdot 2$, instead we count the number of copies of $2$ that should be multiplied together and write that next to the number:
\[ 2^2 = 4 \ \ \ \ \ \ \ \ \ \ 2^3 = 8 \]
\[ 2^4 = 16 \ \ \ \ \ \ \ \ \ \ 2^{10} = 1024 \]
...
The exponent is written to the right and slightly above (it is often made a little smaller in size).  This can be done for \emph{any} number.
\[ 3^2 = 9 \ \ \ \ \ \ \ \ \ \ 3^3 = 27 \ \ \ \ \ \ \ \ \ \ 3^4 = 81 \]
...
\[ 2 \cdot 2 \cdot 2 \cdot 2 \cdot 2 = 2^5 = (2 \cdot 2) \cdot (2 \cdot 2 \cdot 2) = 2^2 \cdot 2^3 \]
...
\[ \frac{8}{4} = \frac{2^3}{2^2} = 2^3 \cdot 2^{-2} \]
\[ = 2^{3-2} = 2^1 = 2 \]
...
In the middle we have the sequence $2 \ 4 \ 8 \ 16$ and the equivalent sequence above it:  $2^1 \ 2^2 \ 2^3 \ 2^4$.  Moving to the right consists of multiplying by $2$ and adding $1$ to the exponent.  But moving to the left consists of dividing by $2$ and subtracting $1$ from the exponent.
...
Here's another thought.  Suppose we imagine that exponents don't have to be just $\{1, 2, 3 \dots \}$ but might be negative integers or fractions or even zero.  We'll see that this forms a consistent system.  So then if
\[ 1 \cdot 2 = 2 \]
\[ 2^p \cdot 2^1 = 2^1 \]
...
$\sqrt{2}$ is a new kind of number called an \emph{irrational} number.  The name means there is no rational number (no fraction) which is exactly equal to $\sqrt{2}$.  We'll see why in another write-up.  
...
Using the addition rule, we see that $p + p = 1$.  Clearly $p = 1/2$ so the square root of $2$ should be written as
...
We say that $0.30103$ is the logarithm of $2$ to the base $10$. ($0.30103$ is only approximately correct.)  
...
\[ \log_b (n) = p \]
or sometimes the parentheses are left out
\[ \log_b n = p \]
