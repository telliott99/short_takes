...
$r$ is the length of the ray extending from the origin to a point on the circle.  For a slice of the circle, the lengths of the two rays bounding the slice are different.  Since $r$ varies with angle $\theta$, we are looking for an expression for $r(\theta)$.  
...
Now we want to do some calculus.  As we've seen, it isn't necessary for the area calculation, but the terms in the solution have obvious counterparts in the geometry we've already done.
...
Technically, this is a double integral over the area element in two variables, but we can replace the inner integral $\int r \ dr$ part by $\frac{r^2}{2}$ (trust me).
...
We'll have more to say about this below.  If we didn't already know how to solve $\int \cos^2 x \ dx$, this would give one.
...
Note on circular caps:  it is tempting to use the same formula to calculate the area of a circular cap as an integral with the angle as the variable, but there is a subtlety.  Namely, this angle is $t$, the complement of $s$.  So the $r(\theta)$ term is not cosine but $\sin t$.  The area is
