...
This is a write-up about quadratic equations, in three parts.  The first is a really gentle introduction, the second gives some more detail about the theory and what is, I hope, an easier way to remember the \emph{quadratic equation}.  The third works through a few basic examples.
...
Since we are going to use the symbol $m$ for something else today, allow me to write $y = kx + y_0$, with $k$ being the slope.
...
and one page even says something else is standard form.  Anyway, for us here, standard form means that we have all three cofactors, $ax^2 + bx + c$.
...
We might notice a couple of things about this graph.  First, we don't have a straight line as with the linear equation, any more but a curve that bends upward.  The graph is said to resemble a "cup" that "opens upward".  If the constant $a$ is negative then the graph will flip directions, and the "cup" opens downward.
...
We will see that the zeros of the equation (the $x$-values where $y = 0$ are evenly spaced on either side of $m$, so we can think of it possibly standing for mean, the average value of the zeros.

In fact, the curve as a whole is symmetric about the vertical line $x = m$.  That line is called the \emph{axis of symmetry}.  If we move some distance $d$ horizontally, so that $x = m \pm d$, the values the function generates, the $y$-values, are always the same.
...
\[ y = x^2 \]
\[ y = ax^2 \]
Notice that in this case, the graph does not really cross the $x$-axis but just touches it at the vertex.  The vertex lies on the $x$-axis and the corresponding $y$-value is $y = 0$.
...
A second form of the quadratic and a specific example:
\[ y = (x - s)(x - t) \ \ \ \ \ \ \ \ \ \ y = (x + 3)(x - 1) \]
...
Don't forget the minus sign.  If the equation is $y + k = (x + h)^2$ the vertex is at $(-h,-k)$ and vice-versa.
...
Then we do whatever the equation says.  And finally that must equal the adjusted value of $y$.

To simplify things, let us first think about the case where the shape factor $a = 1$.  Then
...
Instead, let us mention now a (related) method for finding roots which feels much more intuitive.  In fact, it's so simple, I don't know why it's not usually in the book.

Rewriting the quadratic formula, before, we had that $m = -b/2a$ and then the roots are
\[ x = m \pm \sqrt{m^2 - c/a} \]
...
The roots, call them $s$ and $t$, are symmetric with $m$.
\begin{center} \includegraphics [scale=0.5] {quad3.png} \end{center}

Suppose that $s$ is smaller than $m$ and $t$ is the larger one.  Then, the distance between $s$ and $m$ is
\[ d = m - s \]
\[ s = m - d \]
...
The change in sign happens because $m$ lies \emph{between} $s$ and $t$.  So 
\[ st = (m-d)(m+d) = m^2 - d^2 \]

We want to know $d$.  We already know $m$, and we also know $st$.  Wait, what?  
...
The formal apparatus for dealing with quadratic equations is called the \emph{quadratic formula}.  Here we will derive the formula.
...
Now it's just algebra.  If you recall that $m = -b/2a$ this is the same as
...
Dividing both sides by $t$ in this way, we "lose" a solution, but that solution is $t = 0$, which we don't care about.  Now plug that expression for $t$ back into the $x$-equation.
\[ x = \frac{v_x v_y}{16} \]
...
The maximum comes when $v_x = v_y$, that is, for an angle of $45^{\circ}$.  Note:  this isn't true in real life, because of air resistance.

An even better way to do this is to say, if something is a maximum, then its positive square root is also a maximum.  So let's maximize the square of the previous expression
\[ v_x^2(v^2 - v_x^2) \]

$v_x$ will be some fraction $x$ of $v$.
\[ xv^2 (v^2 - xv^2) = v^2 \ [ \ x(1-x) \ ] \]
$v$ is a non-zero constant. 

This problem really amounts to maximizing $x (1-x)$.  The quadratic is 
\[ -x^2 + x = 0 \]
The maximum is $m = -b/2a = 1/2$.
...
Can you see that pictorially, for a parabola and a line, there are three possibilities:  the line crosses the parabola at two points, or no points, or it just touches the parabola at a single point.  If it sounds like this will generate a quadratic, you're right!
...
The tangent to the curve $ax^2 + bx + c$ has the slope $2ax + b$.  You might guess that the $2$ in the slope comes from the power of $2$ in the parabola, and you'd be right.  But that's calculus.
