...
The golden ratio or mean is a famous proportion between the length and width of a rectangle that "looks nice".  It is derived as follows.

Make a rectangle with length $x$ and width equal to $1$.  The ratio of length to width is then just $x$.

Draw a square using the side of the rectangle with length $x$, right next to the original rectangle.  This forms a larger rectangle with length $x + 1$, and the ratio of length to width for this rectangle is $(x+1)/x$.

For the golden ratio, these two ratios should be the same.  That is
\[ x = \frac{x + 1}{x} \]
\[ x^2 = x + 1 \]
\[ x^2 - x - 1 = 0 \]

We use the simple version of the quadratic formula.  We have $a = 1$, $b = -1$ and $c = - 1$.
\[ m = - \frac{b}{2a} = \frac{1}{2} \]
and the roots are
\[ s,t = m \pm \sqrt{m^2 - c} \]
\[ = \frac{1}{2} \pm \sqrt{\frac{1}{4} + 1} \]
\[ = \frac{1}{2} \pm \frac{\sqrt{5}}{2} \]
The lengths are positive numbers so the ratio should also be positive.  We take the first root.
\[ \phi = \frac{1 + \sqrt{5}}{2}  \]
$\phi$ is the golden ratio and its value is about $1.618$.
...
Here is an approach that runs into problems:  substitute for $t'$ right away.
\[ 12 = (v + 2) t \]
\[ 12 = (v - 2) (8 - t) \]
Multiplying out
\[ 12 = vt + 2t \]
\[ 12 = 8v - vt - 16 + 2t \]
We might add and get rid of $vt$.  Or we could equate the two right-hand sides and get rid of $2t$.  But in either case we will still have both $v$ and $t$ and only a single equation.
