...
We first looked at parts of a quarter circle formed by a sector of central angle $s$ down from the vertical.  

We found that the area of the central part (a \emph{belt}, extending below the horizontal axis) is
\[ s + \sin s \cos s \]

So the circular cap (over the entire half-circle) is $\pi/2$ minus this or
...
So, again, the area of a belt is exactly the same as the area of the corresponding slice.  If the belt or cap is given in terms of a height, $h = \sin s$.
...
We want to do some calculus for this problem, so we must introduce a coordinate system.  One convenient approach is to stay with Cartesian coordinates but displace the circle from the origin.
...
For a slice of the circle, the lengths of the two rays bounding the slice are different.  Since $r$ varies with angle $\theta$, we are looking for an expression for $r(\theta)$.  
...
Another way to get an expression for $r$ in terms of the angle subtended uses the law of cosines.  Recall that for three sides of a triangle $a,b$ and $c$ and angle $C$ between $a$ and $b$ we have:
...
It's a bit of a diversion, but a different perspective is to consider the central angle $\phi = 2 \theta$.  The supplementary angle to $\phi$ (call it $\phi'$) is the apex angle in the same double triangle we had before.  Its cosine is the same magnitude as $\cos \phi$ but with a minus sign.
...
Now we want to do some calculus.  As we've seen, it is unnecessary for the area calculation, but the terms in the solution have obvious counterparts in the geometry we've already done.
...
The figure above gives the area element in two variables, but we can replace the $\int r \ dr$ part by $\frac{r^2}{2}$ (trust me).

The area is
\[ A =  \frac{1}{2} \  \int_0^{\theta} r^2 \ d \theta \]
The factor of one-half comes from the triangular shape.

Using the first expression for $r^2$ the integral is 
...
From the double angle formula for sine.  

We'll have more to say about the first solution, below.  If we didn't already know how to solve $\int \cos^2 x \ dx$, this would give one.

\subsection*{yet another view}
The equation of the unit circle as a function $y = f(x) = \sqrt{1- x^2}$.

Consider a chord of the unit circle, drawn at a height $h$ above the center (measured on the chord's perpendicular bisector).  

For this approach, we must find the two points where the chord meets the circle.  Let's call them $\pm \ x_0$.  These are the two endpoints which are on the circle and where the corresponding $y$-value is $y_0 = h$.
...
This integral is also famous, and is really the same as the one we showed before.
...
So $x = \sin u$, and $\sqrt{1 - x^2} = \cos u$, and also $dx = \cos u \ d u$, the integral is 
...
It is a curious fact that the derivative of the double angle formula for sine gives that for cosine, and vice-versa.

Note on caps:  it is tempting to use the same formula to calculate the area of a circular cap as an integral with the angle as the variable, but there is a subtlety.  Namely, this angle is $t$, the complement of $s$.  So the $r(\theta)$ term is not cosine but $\sin t$.  The area is
\[ 2 \int_0^{t} \sin^2 t \ dt = t - \sin t \cos t \]
which matches what we had before.

Alternatively, let the angle run from $s$ to $\pi/2$ and use $\cos^2 s$, as before.
...
The last view (which is so simple that I'll skip the diagram for a moment), is to turn the circle another quarter-turn.  The area of a belt of height $h$ is simply
\[ A = 2 \int_0^h  \sqrt{1 - x^2} \ dx \]
while the cap requires simply changing the bounds to $h$ and $1$.

We need that factor of $2$ for the answer because the integral is just the area \emph{above} the $x$-axis.
...
The bounds are simply $0$ and $h$.  But since $x=0$ at the lower bound, the whole expression evaluates to $0$ there.  At the upper bound we have
\[ A =  \sin^{-1} h + h \ \sqrt{1 - h^2} \]

What is $h$?  If you look the diagram below you should be able to see that $h = \sin \theta$, so finally
...
So that is one of the terms that we need to match the other answers.  The term with $x_0 y_0/2$ will cancel.

Next, the edge of the circle is (using the integral we solved earlier):
...
Hence
\[ A_{\text{tot}} = A_{\triangle} + A_{\odot} \]
\[ =  \theta - \frac{x_0 y_0}{2} + \sin \theta \cos \theta + \frac{x_0 y_0}{2} \]
\[ = \theta + \sin \theta \cos \theta \]

And, in passing, note that what we have called $A_{\odot}$ is really one-half of a circular cap at height $h = x_0$.  So twice that is the area of the cap, namely
\[ A_{\text{cap}} = \sin^{-1} h  - h \sqrt{1 - h^2} \]

$\square$

Finally, we might explore whether these formulas give the correct answers in other situations.  In the figure below is a slice formed by a right angle plus adjacent angles $\theta + \phi = \pi/2$. 
...
Is that really possible?  The sum of the other two areas must also be $\pi/2$.  The total white area is
...
That looks like a problem, but the term in brackets is $\sin \phi + \theta = \sin \pi/2 = 0$, so we're good.
