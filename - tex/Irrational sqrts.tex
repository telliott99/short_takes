...
This follows since multiplication of two integers $m \cdot n$ is equivalent to repeated addition of $n$ to itself.  We start by adding $n + 0 = s$ and then do $n + s = s';  s = s'$ a total of $m-1$ times.
...
Any odd number can be written as $2k + 1$ (for integer $k$), which squares to give $4k^2 + 4k + 1$, which is an odd number. 
