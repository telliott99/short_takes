...
\subsection*{Solve for u and v algebraically}
We have already given a method to convert these formulas to functions $u = f(x,y)$ and $v = f(x,y)$.

An easy way to do it algebraically is to switch $x,y$ for $u,v$, and at the same time, substitute $-\theta$ for $\theta$.  What this amounts to is relabeling our diagram with $x,y$ being the rotated axes, and then rotating in the opposite direction (cw rather than ccw).
\[ x = u \cos \theta - v \sin \theta \]
switch
