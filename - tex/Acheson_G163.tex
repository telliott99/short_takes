...
We are given only the radii of the small circles be $a,b$ and $c$, and are asked to find $r$, which I have written as $R$.

\begin{center} \includegraphics [scale=0.4] {Acheson_G163.png} \end{center}

\emph{Solution}.

Let the relevant half-angles be $\alpha, \beta$, and $\gamma$.  These are half angles, so they sum to be one right angles.  Therefore, any one angle is complementary to the sum of the other two.  
...
showed that 
\[ \sin \alpha = \frac{R - r}{R + r}, \ \ \ \ \ \ \cos \alpha = \frac{2\sqrt{Rr}}{R + r} \]
