...
Consider a circular pizza pie.  Choose a point \emph{anywhere} in the disk.  Draw two perpendicular chords crossing at the point, with any orientation, and then fill in with two more chords that bisect the angles, giving two pairs of perpendicular chords, rotated at $45^{\circ}$ to each other.

Form the sum of the areas of alternate slices.  Above, the two collections are shaded to tell them apart.  The total dark area is always one-half, equal to the total light area.  The pizza is evenly divided, even though the slices are wonky.
...
For any change $\Delta x$ horizontally to the right, $OQ$ gets smaller and $OP$ gets larger.  Crucially, they change by the same amount, namely $\sqrt{2} \ \Delta x$.  (There is an equal $\Delta y$ to add to $\Delta x$ to give $\Delta OP$ or $\Delta OQ$).  $OPRQ$ is a rectangle (right angles at the vertices), and the sum of two adjacent sides is constant, $OQ + OP = PR + QR$.
...
and so on.  ($c - d$ is the length of a central region straddling the chord center).
...
Look at the terms in the brackets.  The first two components are $\delta_{cd} + \delta_{gh}$ while the third is $- \delta_{ab}$.  Compared to above to see that this is zero, so $\Delta A = 0$.

$\square$

I claim that any point or orientation within the circle can be reached by vertical plus horizontal movements.  Since these do not change the relative area, equality of areas holds.
...
There is a giant mess at the grid center.  Make a drawing of the center blown up large and form pointy extensions.  By symmetry these cancel for each pair of chord arms.
