...
When the value of the function is plotted on the $y$-axis against $x$, we have a parabola opening up.  Furthermore, for $x = 1$, the value of the function is $-1$.  

Although we don't yet know where the vertex is, we do know that it lies below the $x$-axis, so the curve will pass that axis in two places going up.  There are two real roots, values of $x$ resulting in an output of $zero$.
...
Now, if $x$ is irrational and $p/q$ a rational number, then the result $x + p/q$ is irrational.  

\emph{Proof}.  

Suppose $x + p/q$ is rational.  Then $x + p/q$ minus $p/q$ would also be rational, but that's just $x$, which was supposed to be irrational.  This is a contradiction.  Therefore $\sqrt{5} + 1$ is irrational.  

$\square$

A similar argument will show that $(\sqrt{5} + 1)/2 = \phi$ is also irrational. 

However, do not make the mistake of thinking irrational times irrational is irrational, since we can easily come up with a counter-example:  $\sqrt{2} \cdot \sqrt{2} = 2$.
